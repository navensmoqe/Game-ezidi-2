...
let renderer,scene,camera,composer,controls,clock=new THREE.Clock(),quality='high',running=false,won=false;
...
function init(){
  ...
  buildLighting();buildWorld();buildEnemies();buildWeapon();buildPost();bind();animate();
}
...
function buildWorld(){
  ...
  // Yazidi-inspired sun shrine marker (non-sacred stylized environmental landmark)
...
function bind(){
  ...
  ui.start.onclick=startGame;ui.resume.onclick=()=>{ui.pause.classList.add('hidden');controls.lock()};ui.restart.onclick=()=>location.reload();
  document.querySelectorAll('.quality').forEach(b=>b.onclick=()=>{document.querySelectorAll('.quality').forEach(x=>x.classList.remove('active'));b.classList.add('active');quality=b.dataset.quality;applyQuality()});
...
function startGame(){ui.intro.classList.add('hidden');ui.hud.classList.remove('hidden');if(isTouch)ui.mobile.classList.remove('hidden');else controls.lock();running=true;clock.getDelta()}
function showPause(){if(won)return;ui.pause.classList.remove('hidden')}
function bindTouch(){
  const joy=$('#joystick'),stick=$('#stick');let jid=null,cx=0,cy=0;
  joy.addEventListener('pointerdown',e=>{jid=e.pointerId;const r=joy.getBoundingClientRect();cx=r.left+r.width/2;cy=r.top+r.height/2;joy.setPointerCapture(jid)});joy.addEventListener('pointermove',e=>{if(e.pointerId!==jid)return;let dx=e.clientX-cx,dy=e.clientY-cy,d=Math.hypot(dx,dy),m=35;if(d>m){dx*=m/d;dy*=m/d}moveTouch.x=dx/m;moveTouch.y=-dy/m;stick.style.transform=`translate(${dx}px,${dy}px)`});joy.addEventListener('pointerup',()=>{jid=null;moveTouch.x=moveTouch.y=0;stick.style.transform='' });
  renderer.domElement.addEventListener('pointerdown',e=>{if(e.clientX<innerWidth*.42)return;look={id:e.pointerId,x:e.clientX,y:e.clientY}});renderer.domElement.addEventListener('pointermove',e=>{if(e.pointerId!==look.id)return;const dx=e.clientX-look.x,dy=e.clientY-look.y;camera.rotation.order='YXZ';camera.rotation.y-=dx*.0032;camera.rotation.x=Math.max(-1.1,Math.min(1.1,camera.rotation.x-dy*.0032));look.x=e.clientX;look.y=e.clientY});renderer.domElement.addEventListener('pointerup',e=>{if(e.pointerId===look.id)look.id=null});
  $('#fire-mobile').addEventListener('pointerdown',shoot);$('#reload-mobile').onclick=reload;$('#jump-mobile').onclick=jump;
}
function applyQuality(){const q=quality==='high'?1.7:quality==='medium'?1.25:1;renderer.setPixelRatio(Math.min(devicePixelRatio,q));renderer.shadowMap.enabled=quality!=='low';scene.fog.density=quality==='low'?.011:.0085}
